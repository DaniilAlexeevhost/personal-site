          ...
          <div className="grid gap-5 sm:gap-6 md:grid-cols-2 xl:grid-cols-3">
            {paginatedNotes.map((item) => {
              ...
              return (
                ...
                <div className="pointer-events-none flex h-full w-full max-w-[88%] min-w-0 flex-col items-start overflow-wrap-anywhere">
                  <div className="relative z-10 mb-5 flex min-w-0 flex-wrap gap-2.5 text-xs text-zinc-500">
                    <Link
                      href={createTagRoute(item.category)}
                      className="pointer-events-auto max-w-full rounded-full border border-zinc-200 px-3 py-1 transition hover:border-zinc-300 hover:bg-zinc-50 hover:text-zinc-950"
                    >
                        {item.category}
                    </Link>
                    {displayTags[0] && (
